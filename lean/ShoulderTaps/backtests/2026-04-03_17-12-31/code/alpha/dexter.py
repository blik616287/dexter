...
class DexterAlpha(BaseShoulderTapAlpha):
    ...
    def __init__(self):
        ...
        self._live_volume = {}
        # Pending shoulder taps awaiting bar-close confirmation
        # {ticker: {"bar_start": datetime, "result": dict}}
        self._pending_taps = {}
    ...
    def Update(self, algorithm, data):
        ...
        for ticker in self._symbols:
            ...
            # --- Phase 1: Check for bar-close confirmation of pending taps ---
            pending = self._pending_taps.get(ticker)
            if pending is not None and pending["bar_start"] != bar_start:
                # New bar started — check if the COMPLETED bar confirmed the tap
                last_bar = candles[-1]
                last_close = float(last_bar.get("close", 0))
                last_volume = float(last_bar.get("volume", 0))
                result = pending["result"]
                direction = result["direction"]
                ch_high = result["context_values"]["channel_high"]
                ch_low = result["context_values"]["channel_low"]

                # Confirm: bar closed outside the channel
                confirmed = False
                ...
                if confirmed:
                    ...
                    algorithm.Debug(
                        f"[dexter] CONFIRMED {direction} on {ticker} "
                        f"bar closed @ ${last_close:.2f} | {result.get('notes', '')}"
                    )
                    ...
                    algorithm.Debug(
                        f"[dexter] REJECTED {direction} tap on {ticker} "
                        f"— bar closed @ ${last_close:.2f} back inside channel"
                    )

            # --- Phase 2: Shoulder tap on live price (forming bar) ---
            # Skip if we already have a pending tap for this bar
            if ticker in self._pending_taps and self._pending_taps[ticker]["bar_start"] == bar_start:
                continue
            ...
            if result is not None and result.get("triggered"):
                # Register as pending — awaits bar-close confirmation
                self._pending_taps[ticker] = {
                    "bar_start": bar_start,
                    "result": result,
                }

                algorithm.Debug(
                    f"[dexter] TAP {result['direction']} on {ticker} "
                    f"@ ${current_price:.2f} — awaiting bar close | "
                    f"{result.get('notes', '')}"
                )

                # Fire shoulder tap alert (informational, not a trade yet)
                if hasattr(algorithm, '_alert_manager') and algorithm._alert_manager:
                    algorithm._alert_manager.fire_alert(
                        model_name=self._name,
                        symbol=ticker,
                        direction=f"TAP_{result['direction']}",
                        strength=result.get("strength", 50),
                        trigger_values=result.get("trigger_values", {}),
                        context_values=result.get("context_values", {}),
                    )
